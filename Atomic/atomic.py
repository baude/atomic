...
class Atomic(object):
    ...
    def mount(self, mountpoint=None, docker_object=None, live=False):
        try:
            if mountpoint is None:
                mountpoint = self.args.mountpoint
            if docker_object is None:
                docker_object = self.args.image
            if 'live' in self.args:
                live = self.args.live

            options = [opt for opt in self.args.options.split(',') if opt] if 'options' in self.args else ""
            mount.DockerMount(mountpoint, live).mount(docker_object,
                                                                options)
    ...
    def _get_atomic_config(self):
        """
        Loads the atomic configuration file (/etc/atomic) into the var
        self.atomic_config
        :return: None
        """
        ...
        with open(atomic_conf, 'r') as conf_file:
            self.atomic_config = yaml_load(conf_file)
