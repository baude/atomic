...
class DockerBackend(Backend):
    ...
    def prune(self):
        for id in self.get_docker_images(get_all=True, quiet=True, filters={"dangling": True}):
            self.delete_image(id, force=True)
            util.write_out("Removed dangling Image {}".format(id))
        return 0
